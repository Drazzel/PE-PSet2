...
ggarrange(plotlist = list(plot3, plot4, plot5, plot6, plot7))
```
...
reg1 <- data %>% filter(., (bweight >= (1500-85)) & (bweight <= (1500+85))) %>% lm(agedth5~VLBW + VLBWInter + NegVLBWInter, data=.)
summary(reg1)
```
I AIN'T INTERPRETING SHIT, GROGGER!!
...
reg2 <- data %>% filter(., (bweight >= (1500-85)) & (bweight <= (1500+85))) %>% lm(agedth4~VLBW + VLBWInter + NegVLBWInter + as.factor(mom_age_bins) + mom_ed1 + mom_ed2 + mom_ed3 + mom_ed4 + mom_ed5 + as.factor(mom_race) + as.factor(yob) + gest_wks1 + gest_wks2+ gest_wks3 + gest_wks4 + nprenatal_1 + nprenatal_2 + nprenatal_3 + nprenatal_4, data=.)
options(scipen = 999)
summary(reg2)
```
