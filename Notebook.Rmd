...
#create data bins
binbreaks <- c(min(data$bweight)-1,rev(lobins[2:6]), upbins, max(data$bweight)+1)
data$bins <- cut(data$bweight, breaks = binbreaks, labels = 1:12)
...
baltest <- lm(data = databal,bins ~ mom_age + mom_race + mom_ed + gest + nprenatal + sex + agedth)
summary(baltest)
```
...
plot4 <- data %>% group_by(bins) %>% summarize(mean = mean(mom_ed, na.rm = TRUE)) %>% ggplot(aes(bins,mean)) + geom_point() + geom_vline(xintercept = 6.5, size = 2, linetype = "dashed" ,col = "pink")  + theme_classic() + ylab("Education Level Achieved") + xlab("Birth Weight Bins") + ggtitle("Mean Mother's Education (in grade level completed)") + theme(plot.title = element_text(hjust = 0.5))
...
ggarrange(plotlist = list(plot3, plot4, plot5, plot6))
```
...
```{r}
reg2 <- data %>% filter(., (bweight >= (1500-85)) & (bweight <= (1500+85))) %>% lm(agedth4~VLBW + VLBWInter + NegVLBWInter, data=.)
options(scipen = 0)
summary(reg2)
```
